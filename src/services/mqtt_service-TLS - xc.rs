...
pub async fn mqtt_connect(
    ...
    let exe_dir = std::env::current_exe()
        ...
        .parent()
        .ok_or("get exe dir failed")?
        .to_path_buf();

    let ca_path   = exe_dir.join("src-tauri/ca.pem");
    let cert_path = exe_dir.join("src-tauri/client-cert.pem");
    let key_path  = exe_dir.join("src-tauri/client-key.key");

    // 2. 读取文件
    let ca   = tokio::fs::read(ca_path).await.map_err(|e| e.to_string())?;
    let cert = tokio::fs::read(cert_path).await.map_err(|e| e.to_string())?;
    let key  = tokio::fs::read(key_path).await.map_err(|e| e.to_string())?;
