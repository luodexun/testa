...
pub async fn mqtt_connect(
    ...
    // 1) 读取证书
    let ca = tokio::fs::read("ca.pem").await.map_err(|e| e.to_string())?;
    let cert = tokio::fs::read("client-cert.pem").await.map_err(|e| e.to_string())?;
    let key  = tokio::fs::read("client-key.key").await.map_err(|e| e.to_string())?;
